...
| `get_credential_live_status(number)`        | RPC (SQL, `SECURITY DEFINER`)     | **Internal (retired)**    | No caller. Since migration `00000000000090`: `EXECUTE` revoked from `PUBLIC`, `anon` and `authenticated` (service_role only); body limited to active staff of the credential's own woreda. Use `verify_credential_token` instead.                                                                                                                |
