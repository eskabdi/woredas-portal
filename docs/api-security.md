...
| `verify_credential_token(token)`            | RPC (SQL, `SECURITY DEFINER`)     | **Public**                | Deliberately anonymous — called by `src/routes/v.$token.tsx` after a client-side signature check, for live revocation status only. Takes no identity, returns no more than the printed card already shows. Since migration `00000000000091` a token matches on its signed payload plus the signature's r, so every re-encoding of one card resolves to that card. |
...
| `rental_eligibility(resident, house, type)` | RPC (plpgsql, `SECURITY DEFINER`) | **Private**               | Called by `woreda.rental-houses.occupants.new.tsx` and by the `zz_enforce_rental_eligibility` trigger. Since migration `00000000000090`: not executable by `anon`; a signed-in non-super-admin caller needs a `rental.*` permission and only sees residents and houses in their own woreda (a resident elsewhere reads as `resident_not_found`).                  |
| `get_credential_live_status(number)`        | RPC (SQL, `SECURITY DEFINER`)     | **Internal (retired)**    | No caller. Since migration `00000000000090`: `EXECUTE` revoked from `PUBLIC`, `anon` and `authenticated` (service_role only); body limited to active staff of the credential's own woreda, which a service_role call never is, so it now returns `NULL` for every caller. Use `verify_credential_token` instead.                                                  |
